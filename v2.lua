...
local function createSilentAimButton()
    ...
    Button.Name = "SilentAimButton"
    Button.Parent = ScreenGui
    Button.Size = UDim2.new(0, 120, 0, 45)
    Button.Position = UDim2.new(0.5, -60, 0.75, 0)
    Button.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    Button.BackgroundTransparency = 0.3
    Button.Text = "Silent Aim"
        ...
        if isActive then
            ...
            connection = game:GetService("RunService").Heartbeat:Connect(function()
                pcall(function()
                    ...
                    if not gun then
                        local gunInBackpack = t13.lp.Backpack:FindFirstChild("Gun")
                    ...
                    local role = getRole(t13.lp)
                    if role ~= "Sheriff" and role ~= "Hero" then return end
                    if isDead(t13.lp) then return end
                    
                    local murderer = whosthemurdererson()
                    if not murderer then return end
                    
                    local murdererChar = murderer.Character
                    if not murdererChar then return end
                    
                    local targetPart = murdererChar:FindFirstChild("UpperTorso")
                    if not targetPart then
                        targetPart = murdererChar:FindFirstChild("Torso")
                        if not targetPart then
                            targetPart = murdererChar:FindFirstChild("Head") or murdererChar:FindFirstChild("HumanoidRootPart")
                        end
                    end
                    ...
                    local gunRemote = t13.w:WaitForChild(t13.lp.Name, 2)
                    if not gunRemote then return end
                    
                    local gunReplica = gunRemote:FindFirstChild("Gun")
                    if not gunReplica then return end
                    
                    local shootEvent = gunReplica:FindFirstChild("Shoot", true)
                    if not shootEvent then return end
                    
                    shootEvent:FireServer(
                        CFrame.new(rootPart.Position, predictedPos),
...
-- Создаем кнопку
task.wait(0.5)
local silentAimBtn = createSilentAimButton()
-- ========== КОНЕЦ SILENT AIM ==========
